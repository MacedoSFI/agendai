...
export default function Layout() {
  ...
  const checkNew = useCallback(async () => {
    try {
      ...
      const count = data.filter(
        (a) => new Date(a.created_at) > new Date(lastCheck),
      ).length;
      setNewCount(count);
    } catch {}
  }, [lastCheck]);
  ...
  const navItems = [
    ...
    { to: "/app/appointments", icon: "📋", label: "Agendamentos", badge: newCount },
  ];
  ...
  return (
    <NavLink
      ...
      style={({ isActive }) => ({
        ...
        background: isActive
          ? "linear-gradient(135deg, rgba(124,106,247,.2), rgba(79,209,197,.1))"
          : "transparent",
        border: isActive
          ? "1px solid rgba(124,106,247,.3)"
    ...
    >
      ...
      {badge > 0 && (
        <span style={{ background: '#f85149', color: '#fff', borderRadius: 10, fontSize: 10, fontWeight: 700, padding: '2px 7px', minWidth: 18, textAlign: 'center' }}>
          {badge > 9 ? '9+' : badge}
        </span>
      )}
    </NavLink>
