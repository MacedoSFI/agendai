...
export default function SettingsPage() {
  ...
  return (
    <>
      ...
      {/* coluna única sempre — cards empilhados */}
      <div style={{ display:'flex', flexDirection:'column', gap:20, maxWidth: isMobile ? '100%' : 860 }}>

        {/* ── MEU PLANO ── */}
        <div id="plano" style={card}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                  <div style={{ background:'#1c1c26', borderRadius:10, padding:'14px 16px' }}>
                    ...
                    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:12, flexWrap:'wrap' }}>
                      <span style={{ fontSize:14, color:'#e8e8f0', fontWeight:600, wordBreak:'break-all' }}>{PIX_KEY}</span>
                      <button onClick={() => { navigator.clipboard.writeText(PIX_KEY); showToast('✅ Chave Pix copiada!'); }}
                        style={{ padding:'6px 12px', background:'rgba(124,106,247,.2)', border:'1px solid rgba(124,106,247,.3)', borderRadius:6, color:'#a89cf7', fontSize:12, cursor:'pointer', flexShrink:0 }}>
                        Copiar
                      </button>
                    </div>
                  </div>
        ...
        <div style={card}>
          ...
          <form onSubmit={handleSaveProfile} style={{ padding:20, display:'flex', flexDirection:'column', gap:14 }}>
            ...
            <div><label style={labelStyle}>Nome do Negócio</label>
              <input style={inputStyle} value={form.business_name} onChange={e=>setForm({...form,business_name:e.target.value})} placeholder="Barbearia Silva..."/>
            </div>
            ...
            <div><label style={labelStyle}>Telefone</label>
              <input style={inputStyle} value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})} placeholder="(11) 99999-9999"/>
            </div>
            <div><label style={labelStyle}>Email</label>
              <input style={{...inputStyle,opacity:.5}} value={user?.email||''} disabled/>
            </div>
...
const btnPrimary = { padding:'10px 16px', borderRadius:8, fontSize:13, fontWeight:500, cursor:'pointer', border:'none', background:'#7c6af7', color:'#fff', fontFamily:'DM Sans,sans-serif', width:'100%' };
